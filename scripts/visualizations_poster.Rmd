...
table_data <- forest_data %>%
  ...
  select(variable_label, OR_formatted, IC_95, p_formatted, Significant) %>%
  rename(
...
tab_results <- table_data %>%
  ...
  tab_style(
    ...
    locations = cells_body(columns = "Odds Ratio", rows = Significant == "Yes")
  ) %>%
  cols_align(
    align = "center",
    columns = c("Odds Ratio", "95% CI", "p-value", "Significant")
  ) %>%
  tab_options(
    table.font.size = 12,
