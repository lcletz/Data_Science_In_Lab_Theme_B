...
We now perform formal statistical tests to confirm our visual observations. The goal is to test whether there is a significant association between each variable and the missingness of RRT_ever.

### Chi-squared tests for categorical variables

We use Chi-squared tests to assess the association between categorical variables and RRT missingness. A significant p-value (< 0.05) suggests that the variable may be related to the missing data mechanism.

```{r chi-squared-tests}
vars_cat <- c(
  "SEX", "INCL_SEPSIS_YN", "AGE_CLASS", "INCL_AKIN", "ARM_NUM",
  "ATCD_Smoking", "ATCD_Diabetes", "ATCD_Chronic_hypertension",
  "ATCD_Cheart_failure", "ATCD_Ckidney_disease", "SOFA_renal_H0",
  "ATCD_Severe_liver_insufficiency", "ATCD_Cirrhosis", "ATCD_Immunocompromised",
  "DEATH28"
)

for (v in vars_cat) {
  cat("\n\nVariable :", v, "\n")
  print(table(data[[v]], data$RRT_MD, useNA = "ifany"))
  print(chisq.test(table(data[[v]], data$RRT_MD)))
}
...
For continuous variables, we use the Wilcoxon rank-sum test. 

```{r wilcoxon-tests}
wilcox.test(INCL_LAB_serum_creatinine ~ RRT_MD, data = data)
wilcox.test(INCL_LAB_blood_urea_nitrogen ~ RRT_MD, data = data)
```
